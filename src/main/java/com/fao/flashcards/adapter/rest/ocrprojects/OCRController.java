package com.fao.flashcards.adapter.rest.ocrprojects;

import com.fao.flashcards.application.model.*;
import com.fao.flashcards.application.port.dto.*;
import com.fao.flashcards.application.port.exception.OCRProcessingException;
import com.fao.flashcards.application.service.FileUploadService;
import com.fao.flashcards.application.service.OCRService;
import com.fao.flashcards.application.service.ProjectService;
 ...
 * Stellt Endpoints für OCR-Operationen, Text-Extraktion und -Bearbeitung
 * bereit.
 */
@RestController
...
public class OCRController {
    ...
    public ResponseEntity<OCRProcessResponse> extractTextFromFile(
        ...
        try {
            ...
            log.error("OCR-Verarbeitungsfehler für Datei {}: {}", fileId, e.getMessage(), e);
    ...
    public ResponseEntity<OCRResultDTO> getOCRStatus(@PathVariable("fileId") String fileId) {
        ...
        try {
            ...
            ProjectFile fileInfo = fileUploadService.getFileInfo(fileId);
            List<OCRResult> projectOCRResults = ocrService.getProjectOCRResults(fileInfo.getProject().getId());
            OCRResult ocrResult = projectOCRResults.stream().filter(result -> result.getExtractedText().getProjectFile()
                    .getId().equals(fileId)).findFirst()
                    .orElseThrow(() -> new EntityNotFoundException("OCRResult nicht gefunden für Datei: " + fileId));

            log.debug("OCR-Status für Datei {}: {}", fileId, ocrResult.getStatus());
            return ResponseEntity.ok(new OCRResultDTO(ocrResult.getStatus()));

        } catch (EntityNotFoundException e) {
            log.warn("Datei nicht gefunden für Status-Abfrage: {}", fileId, e);
            return ResponseEntity.notFound().build();
        } catch (Exception e) {
    ...
    /**
     * GET /api/v1/ocr/project/{projectId}/texts - Alle extrahierten Texte eines
     * Projekts
     */
    @GetMapping("/project/{projectId}/texts")
    public ResponseEntity<List<ExtractedTextDTO>> getProjectExtractedTexts(
    ...
    public ResponseEntity<OCRService.OCRProjectStatistics> getProjectOCRStats(
            @PathVariable("projectId") String projectId) {
    ...
    /**
     * GET /api/v1/ocr/project/{projectId}/unprocessed - Unverarbeitete Dateien
     * eines Projekts
     */
